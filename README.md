...
```js
const {calculateChabadZmanim, roundToNearestMinute} = require('@kehila/zmanim');
...
Missing solar events return `null`. In particular, polar regions with no true sunrise or sunset need rabbinic guidance; the library does not invent a time. Call `roundToNearestMinute()` only at display time. Chabad.org recommends leaving at least a two-minute practical margin around published zmanim.

## Installation from GitHub

```sh
npm install @kehila/zmanim@github:mmr94/kehila-zmanim#v1.0.0
```
